...
Networks.livenet = Networks.add(require('coininfo')('MONA').toBitcore());
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for monacoind is necessary for this upgrade with the "reindex=1" monacoin.conf option. \n' +
                 'There are changes necessary in both monacoin.conf and bitcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/monaco-ex/bitcore-node/blob/monacoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/monacoind')
            }
          }
        }
      };
